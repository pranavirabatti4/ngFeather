...
export function addStyles(project: workspaces.ProjectDefinition, targetName: string, host: Tree, availableAssetPaths: availablePaths, projectName: string, extension?: string): Tree {
    ...
    if (!styles || (styles instanceof Array && !styles.length)) {
      targetOptions = addEmptyStyles(targetOptions, availableAssetPaths);
      return setUpdatedTargetOptions(host, targetOptions, targetName, projectName);
    }
    const existingStyles = styles.map((s) => typeof s === 'string' ? s :  (s as { input: string }).input);
    const styleFilePath = getProjectStyleFile(existingStyles, extension) || '';
    const styleFileExtension = normalizeExtension(path.extname(styleFilePath), DEFAULT_STYLE_EXTENSION, extension);
    const styleFilePatch = (availableAssetPaths as any)[styleFileExtension]?.[0];
...
function addEmptyStyles(targetOptions: Record<string, string | number | boolean | JsonArray | JsonObject>, availableAssetPaths: availablePaths) {
...
function setUpdatedTargetOptions(host: Tree, targetOptions: Record<string, string | number | boolean | JsonArray | JsonObject>, targetName: string, projectName: string): Tree {
...
function normalizeExtension(pathExtension: string, defaultValue: string, extension?: string): string {
  if (!pathExtension) {
    return defaultValue;
  }
